-- ============================================
-- COMPLETE DATABASE SETUP FOR STUDYSYNC (FIXED)
-- ============================================
...
-- ============================================
-- SAMPLE DATA (same as before – insert all)
-- ============================================
...
-- Additional seed data for 3 users
...
JOIN Semesters s ON s.semester_id = c.semester_id
WHERE s.student_id IN (@User1, @User2, @User3)
  AND NOT EXISTS (SELECT 1 FROM BookMaterial bm WHERE bm.course_id = c.course_id AND bm.title = c.course_name + ' - Core Concepts');
...
JOIN Semesters s ON s.semester_id = c.semester_id
WHERE s.student_id IN (@User1, @User2, @User3)
  AND NOT EXISTS (SELECT 1 FROM BookMaterial bm WHERE bm.course_id = c.course_id AND bm.title = c.course_name + ' - Practice Workbook');
...
JOIN Semesters s ON s.semester_id = c.semester_id
WHERE s.student_id IN (@User1, @User2, @User3)
  AND NOT EXISTS (SELECT 1 FROM BookMaterial bm WHERE bm.course_id = c.course_id AND bm.title = c.course_name + ' - Exam Prep Notes');
...
PRINT 'Database StudySync created successfully with all tables and sample data!';
GO
