    ...
    <div className="dashboard-container">
      ...
      <div className="nav-bar flex-between">
        <div style={{ display: 'flex', alignItems: 'center' ,gap: '20px'}}>
          ...
          <Link to="/course-grades">Course Grades</Link>
          <Link to="/incomplete-courses">Incomplete Courses</Link>
          <Link to="/admin/average-gpa">Average GPAs</Link>
                ...
                {courses.map(course => (
                  <div key={course.course_id} className="mini-course-item">
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      ...
                      <button
                        className="text-btn"
                        onClick={() => navigate(`/courses/${course.course_id}/grade`)}
                        style={{ fontSize: '11px' }}
                      >
                        {course.letter_grade ? 'Edit Grade' : 'Enter Grade'}
                      </button>
                    </div>
                  </div>
                ))}
      ...
      <style>{`
        ...
        .mini-course-item { display: flex; justify-content: space-between; align-items: center; padding: 8px 0; font-size: 14px; border-bottom: 1px dashed #eee; }
