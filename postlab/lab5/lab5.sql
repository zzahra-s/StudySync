...
--Find the villain with the most family members who were defeated in battle.
WITH VillainFamily AS (
    SELECT 
        v.CreatureID AS VillainID,
        f.Creature2ID AS FamilyMemberID
    FROM Creature v
    JOIN FamilyRelation f ON v.CreatureID = f.Creature1ID
    WHERE v.Type = 'Villain'
    UNION ALL
    SELECT 
        v.CreatureID AS VillainID,
        f.Creature1ID AS FamilyMemberID
    FROM Creature v
    JOIN FamilyRelation f ON v.CreatureID = f.Creature2ID
    WHERE v.Type = 'Villain'
),
DefeatedFamily AS (
    SELECT vf.VillainID, vf.FamilyMemberID
    FROM VillainFamily vf
    JOIN Creature c ON vf.FamilyMemberID = c.CreatureID
    WHERE c.Status = 'Dead'
)
SELECT TOP 1
    v.Name AS VillainName,
    COUNT(df.FamilyMemberID) AS DefeatedFamilyCount
FROM Creature v
LEFT JOIN DefeatedFamily df ON v.CreatureID = df.VillainID
WHERE v.Type = 'Villain'
GROUP BY v.Name
ORDER BY DefeatedFamilyCount DESC;

--Find the strongest dead hero by counting their wins before death.
WITH DeadHeroes AS (
...
HeroWins AS (
    SELECT dh.CreatureID, dh.Name, COUNT(f.WinnerID) AS Wins
    FROM DeadHeroes dh
    LEFT JOIN Fight f ON dh.CreatureID = f.WinnerID
    GROUP BY dh.CreatureID, dh.Name
)
SELECT TOP 1
    Name AS StrongestDeadHero,
    Wins AS TotalWins
FROM HeroWins
ORDER BY Wins DESC;